...
def parse_args():
  ...
  """
    ...
    parser.add_argument('--c_coeff', help='cl coeff', default=1.0, type=float)
...
height=256 # change if possible
width=320 # change if possible
n_classes=7
...
f_aug = ''
bs = args.bs
print(f"Aug is {aug} and prob is {p}")
for i in range(len(X_train)):
    if random.random()>=p:
        ...
        if aug == 'np_style':
            ...
            X_train[i] = np_style(X_train[i], X_train[i:i+bs])
            if f_aug == 'f_mixup':
                X_train[i], y_train[i] = mixup(xt, yt, X_train[i].copy(), y_train[i].copy())
...
coeff = args.c_coeff #1.0 # 0.01 for autoaug

def custom_loss(y_true, y_pred, x1, is_val=False):
    cat_loss = CategoricalCrossentropy()(y_true, y_pred) 

    if not is_val:
        x1 = x1 / tf.reduce_sum(x1, axis=-1, keepdims=True)
        y_pred = y_pred / tf.reduce_sum(y_pred, axis=-1, keepdims=True)
        epsilon = 1e-7
        kl_loss = tf.reduce_sum(x1 * tf.math.log((x1 + epsilon) / (y_pred + epsilon)), axis=-1)
        kl_loss = tf.reduce_mean(kl_loss)
        net_loss = cat_loss + coeff * kl_loss
    
        return cat_loss, kl_loss, net_loss
    else:
        return cat_loss
...
train_loss_metric_1 = tf.metrics.Mean()
train_loss_metric_2 = tf.metrics.Mean()
train_accuracy_metric = CustomAccuracy()
val_acc_metric = CustomAccuracy()
num_epochs = args.epochs #30
print(f"lr is {lr}, bs is {bs}, coeff is {coeff}, num_epochs is {num_epochs}")
...
for epoch in range(num_epochs):
    ...
    for i in range(trlim):  # Iterate through batches
        with tf.GradientTape() as tape:
            ...
            logits_c = unet_model1(x_batch, training=True)
            cat_loss, kl_loss, loss_value = loss_fn(y_batch, logits, logits_c)
            t_l += loss_value
        ...
        train_loss_metric_1.update_state(cat_loss)
        train_loss_metric_2.update_state(kl_loss)
        train_accuracy_metric.update_state(y_batch, logits)
        
    t_l = t_l/trlim
    train_loss_list.append(t_l)

    print(f"Epoch {epoch+1}: Loss={train_loss_metric.result(), train_loss_metric_1.result(), train_loss_metric_2.result()}, Accuracy={train_accuracy_metric.result()}")
    ...
    for j in range(len(X_test)):
        ...
        val_loss_value = loss_fn(y_batch_test, val_logits, val_logits, is_val=True)
        v_l += val_loss_value
...
if args.model == 'unet':
    lmod[-1].save(f'/content/drive/MyDrive/ood_seg/m1_{aug}_{f_aug}.h5') # change if possible
elif args.model == 'vgg_unet1':
    lmod[-1].save(f'/content/drive/MyDrive/ood_seg/m2_{aug}_{f_aug}.h5') # change if possible
elif args.model == 'segnet1':
    lmod[-1].save(f'/content/drive/MyDrive/ood_seg/m3_{aug}_{f_aug}.h5') # change if possible
elif args.model == 'vgg_segnet1':
    lmod[-1].save(f'/content/drive/MyDrive/ood_seg/m4_{aug}_{f_aug}.h5') # change if possible
else:
    raise Exception
...
ax.legend(loc='best')
if args.model == 'unet':
    plt.savefig(f'/content/drive/MyDrive/ood_seg/m1_{aug}_{f_aug}.png') # change if possible
elif args.model == 'vgg_unet1':
    plt.savefig(f'/content/drive/MyDrive/ood_seg/m2_{aug}_{f_aug}.png') # change if possible
elif args.model == 'segnet1':
    plt.savefig(f'/content/drive/MyDrive/ood_seg/m3_{aug}_{f_aug}.png') # change if possible
elif args.model == 'vgg_segnet1':
    plt.savefig(f'/content/drive/MyDrive/ood_seg/m4_{aug}_{f_aug}.png') # change if possible
else:
    raise Exception
